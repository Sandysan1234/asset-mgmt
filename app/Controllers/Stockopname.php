...
class Stockopname extends BaseController
{
    ...
    public function cekAssetByNo()
    {
        ...
        if (!$noAsset) {
            return $this->response->setJSON([
                'status' => 'error',
                'message' => 'No Asset tidak diberikan'
            ]);
        }

        // Cari di database berdasarkan no_asset
        $assetModel = new \App\Models\AssetModel(); // sesuaikan nama model
        $asset = $assetModel->where('no_asset', $noAsset)->first();
